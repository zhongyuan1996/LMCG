        ...
        if from_flax:
            ...
            if low_cpu_mem_usage:
                ...
                if device_map is None and not is_sharded:
                    param_device = "cpu"
                    # Compatibility: older diffusers' `load_state_dict` doesn't accept `variant`.
                    # We already applied `_add_variant(...)` when resolving `model_file`.
                    ...
                    # Compatibility: older diffusers' `load_model_dict_into_meta` doesn't accept `device=...`.
                    # Use a trivial device_map instead.
                    ...
                    if len(unexpected_keys) > 0:
                        # Compatibility: some diffusers versions may return `None` entries here.
                        unexpected_keys_str = [str(k) for k in unexpected_keys if k is not None]
                        logger.warning(
                            f"Some weights of the model checkpoint were not used when initializing {cls.__name__}: \n {[', '.join(unexpected_keys_str)]}"
                        )
