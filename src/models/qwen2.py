...
def _qwen2_decoder_layer_forward_checkpoint(
    decoder_layer,
    hidden_states,
    attention_mask,
    position_ids,
...
class Qwen2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        position_embeddings: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,  # will become mandatory in v4.46
        ffn_route: Optional[torch.LongTensor] = None,
        **kwargs,
        ...
        # Cross-attention adapter (injected dynamically for medical report generation)
        if hasattr(self, 'cross_attn_adapter') and getattr(self, '_cross_attn_ctx', None) is not None:
            hidden_states = self.cross_attn_adapter(hidden_states, self._cross_attn_ctx)

        # Fully Connected
        residual = hidden_states
        hidden_states = self.post_attention_layernorm(hidden_states)
        if ffn_route is not None:
            try:
                hidden_states = self.mlp(hidden_states, ffn_route)
            except TypeError:
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(
                    _qwen2_decoder_layer_forward_checkpoint,
                    decoder_layer,
                    hidden_states,
                    causal_mask,
                    position_ids,
