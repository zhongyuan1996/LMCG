...
class Showo2Qwen2_5(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        T = 0
        ffn_route = kwargs.get("ffn_route", None)
        if image_latents is None:
            ...
            # --- Cross-attention context for report generation ---
            # Detach context to avoid gradient-checkpoint conflicts: the pool
            # and memory params learn via the adapter's own projections.
            if getattr(self, '_cross_attn_pool', None) is not None:
                with torch.no_grad():
                    _xattn_img = image_embeds.detach()
                    _xattn_ctx = self._cross_attn_pool(_xattn_img)

                    # Relational memory: process context visits sequentially
                    if getattr(self, '_relational_memory', None) is not None:
                        n_imgs = _xattn_img.size(0)
                        if n_imgs > 1:
                            _mem = self._relational_memory.get_initial_memory(
                                1, _xattn_img.device, _xattn_img.dtype,
                            )
                            for _vi in range(n_imgs - 1):  # context visits only
                                _vf = _xattn_img[_vi : _vi + 1].mean(dim=1, keepdim=True)
                                _mem = self._relational_memory(_vf, _mem)
                            _xattn_ctx = torch.cat([_mem, _xattn_ctx], dim=1)

                for _layer in self.showo.model.layers:
                    if hasattr(_layer, 'cross_attn_adapter'):
                        _layer._cross_attn_ctx = _xattn_ctx
            ...
            extra_attentions = outputs.attentions if bool(output_attentions) else None
            ...
            if text_labels is not None and image_labels is not None:
                ...
                if extra_attentions is not None:
                    return logits, loss_ntp, loss_flow, extra_attentions
                return logits, loss_ntp, loss_flow

            elif image_labels is not None:
                loss_flow = velocity_prediction(v_pred, new_image_labels[:v_pred.shape[0]], image_masks)
                if extra_attentions is not None:
                    return logits, loss_flow, extra_attentions
                return logits, loss_flow

            elif text_labels is not None:
                loss_ntp = next_token_prediction(logits, text_labels, self.config.llm_vocab_size)
                if extra_attentions is not None:
                    return logits, loss_ntp, extra_attentions
                return logits, loss_ntp
    ...
    def lm_generate(
        ...
        Most likely you'll want to make sure to be in model.eval() mode of operation for this.
        """
        generated_tokens = list(input_ids or [])
        output_tokens = []
        for _ in range(max_new_tokens):
            ...
            # Greedy if temperature <= 0 (avoids div-by-zero and reduces token-salad in inference).
            if temperature is None or float(temperature) <= 0.0:
                next_token = int(torch.argmax(next_token_logits, dim=-1).item())
                generated_tokens.append(next_token)
                output_tokens.append(next_token)
                if tokenizer is not None and (next_token == tokenizer.eos_token_id or next_token == boi_token):
                    break
                continue
            ...
            if tokenizer is not None and (next_token == tokenizer.eos_token_id or next_token == boi_token):  # EOS token ID
                break
        ...
        for _ in range(max_new_tokens):
            ...
            if attention_mask is not None:
                ...
                attention_mask_a = torch.hstack(
                    [
                        attention_mask,  # L, L
                        torch.zeros((L, 1), device=device, dtype=attention_mask.dtype) + torch.finfo(attention_mask.dtype).min,
                    ]
                )
                attention_mask_b = torch.vstack(
                    [
                ...
                attention_mask = attention_mask_b.to(image_latents.dtype).unsqueeze(0)
