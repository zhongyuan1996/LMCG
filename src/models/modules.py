...
class ModulatedAttentionBlock(nn.Module):
    def __init__(self, config: DiffusionHeadConfig, layer_idx: int):
        ...
        # CRITICAL: Initialize gate biases to 1.0 so gradients flow from step 1
        # Without this, gates=0 at init → diffusion head is identity → no gradient to attn/mlp
        # bias layout: [shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp]
        with torch.no_grad():
            H = config.hidden_size
            b = self.adaLN_modulation[1].bias
            b[2*H:3*H].fill_(1.0)   # gate_msa = 1.0
            b[5*H:6*H].fill_(1.0)   # gate_mlp = 1.0
